...
RETURNS TABLE
AS
RETURN (SELECT Employee.[Name], Employee.Phone, EmployeeInfo.Home FROM Employee
...
RETURNS TABLE
AS
RETURN (SELECT Employee.[Name], Employee.Phone, EmployeeInfo.Birthday FROM Employee
		JOIN EmployeeInfo ON Employee.EmployeeID = EmployeeInfo.EmployeeID
		WHERE Maried = 0);

SELECT * FROM [Second]()

CREATE FUNCTION Third() 
RETURNS TABLE
AS
RETURN (SELECT Employee.[Name], Employee.Phone, EmployeeInfo.Birthday FROM Employee
	JOIN EmployeeInfo ON Employee.EmployeeID = EmployeeInfo.EmployeeID
